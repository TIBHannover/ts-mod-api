		...
		try {
			...
			for (JsonElement ontology : ontologies) {
				...
				if (semanticArtefactCatalogRecord != null) {
					semanticArtefactCatalogRecord.setSemanticArtefactCatalogRecordId(request.getBaseUrl() + "/" + ontology.getAsJsonObject().get("ontologyId").getAsString());
			...
			if (!semanticArtefactCatalogRecords.isEmpty()) {
				if(request.getFormat().equals(FormatOption.jsonld)) {
					responseDto.setJsonResult(semanticArtefactCatalogRecords);
					...
					responseDto.setOtherFormatResult(semanticArtefactCatalogRecords);
				}
